...
class Logkaryawan_model extends Model
{
    ...
    {
        $column_order = array('pegawai.`pegawai_nip`', 'pembagian3.`pembagian3_nama`', 'pegawai.`pegawai_nama`', 'pembagian2.`pembagian2_nama`', 'att_log.`scan_date`', 'att_log.`pin`');
        $column_search = array('pegawai.`pegawai_nip`', 'pembagian3.`pembagian3_nama`', 'pegawai.`pegawai_nama`', 'pembagian2.`pembagian2_nama`', 'att_log.`scan_date`', 'att_log.`pin`');
        $order = array('att_log.`scan_date`' => 'desc');

        $this->select("pegawai.`pegawai_nip` as idkar, pembagian3.`pembagian3_nama` as vendor, pegawai.`pegawai_nama` as nama, pembagian2.`pembagian2_nama` as bagian, IF(att_log.`inoutmode`=1, 'IN', IF(att_log.`inoutmode`=2, 'OUT', IF(att_log.`inoutmode`=3, 'IN', IF(att_log.`inoutmode`=4, 'OUT', IF(att_log.`inoutmode`=5, 'IN', IF(att_log.`inoutmode`=6, 'OUT', IF(att_log.`inoutmode`=7, 'IN', IF(att_log.`inoutmode`=8, 'OUT', IF(att_log.`inoutmode`=9, 'IN', 'OUT'))))))))) AS inouttype, att_log.`scan_date` AS scandate, pegawai.`grup_t` as grup_t, att_log.`pin` AS pin");
